...
for i=1:+1:(N*2) 
    if in(i,1) == 1
        %fprintf("The first IN is at the row: %g \n", i);
        first_in = i;
    break
    end
end
for j=(N*2):-1:1
    if out(j,1) == 1
        %printf("The last OUT is at the row: %g \n", j);
        last_out = j;
    break
    end
end
...
Naverage = W / T;
% Probability of having a response time less than 30 sec
PR1 = sum(Rt < "00:00:30") / C;
% Probability of having a response time less than 3 min
PR5 = sum(Rt < "00:03:00") / C;
...
% Probability of having 0 parts in the machine
PN0 = sum((evs(:,3) == 0) .* evs(:,4)) / T;
% Probability of having 1 part in the machine
PN1 = sum((evs(:,3) == 1) .* evs(:,4)) / T;
% Probability of having 2 parts in the machine
PN2 = sum((evs(:,3) == 2) .* evs(:,4)) / T;

% Probability of having inter-arrival time shorter than 1 min
for i = 1: +1: (N-1)
...
fprintf(1, "Pr(0 parts in the machine): %g\n", PN0);
fprintf(1, "Pr(1 part in the machine): %g\n", PN1);
fprintf(1, "Pr(2 parts in the machine): %g\n", PN2);
fprintf(1, "Pr(Response time < 30 sec): %g\n", PR1);
